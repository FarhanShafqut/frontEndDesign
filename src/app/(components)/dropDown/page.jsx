...
const DropDown = () => {
    ...
    return (
        // <div className={isOpen ? 'pb-20  '  : ''}>
          <div className={`transition-all duration-300 ease-in ${isOpen ? 'mb-20' : ''}`}>
        ...
        {isOpen && (
          <div className='absolute ps-14 flex flex-col items-start w-full transition duration-300 '>
            {drop.map((e, i) => (
              <div key={i} className='hover:bg-slate-600 my-1 w-40 cursor-pointer border-l-transparent hover:border-l-white border-l-4'>
                <h3>{e.title}</h3>
              </div>
            ))}
          </div>
